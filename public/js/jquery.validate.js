	...
	defaults: {
		...
		highlight: function(element, errorClass, validClass) {
			if (element.type === 'radio') {
				...
				//$(element).css({'border-color':'red','outline':'1px solid red'});
			} else {
				if(element.message!="")
		...
		unhighlight: function(element, errorClass, validClass) {
			if (element.type === 'radio') {
				...
				//$(element).css({'border-color':'#4682B4','outline':'none'});
			} else {
				$(element).removeClass(errorClass).addClass(validClass);
				/*arun*/
				//$(element).css({'border-color':'#4682B4'});
			}
		}
	},
		...
		showLabel: function(element, message) { 
			...
			if ( label.length ) {
				...
				if(element.type=="select-one"){
					label = $('<' + this.settings.errorElement + ' style="margin-left:0px" align="right"/>')
					...
					if($(element).next('p').length<=0)
						$(element).after('<p></p>');
					label = $(element).next('p')
							.attr({"for":  this.idOrName(element), generated: true})
							.css({"width":$(element).css('width'),"marginLeft":$(element).css('marginLeft')})
							.removeAttr('class')
							.addClass(this.settings.errorClass)
							.html(message || "");
				}
